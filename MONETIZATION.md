# 💰 MONETIZATION.md

# Cadance

> Modèle économique et stratégie commerciale

Version : 0.1.0

---

# 📖 Introduction

Cadance est pensé comme un SaaS professionnel destiné aux chauffeurs VTC indépendants.

Le modèle économique repose principalement sur un abonnement mensuel permettant aux chauffeurs d'accéder à des fonctionnalités avancées.

L'objectif n'est pas de vendre une simple application, mais un véritable outil métier.

---

# 🎯 Objectifs commerciaux

Créer un modèle :

- simple à comprendre
- accessible aux indépendants
- rentable sur le long terme
- évolutif vers les entreprises VTC

---

# 💡 Positionnement

Cadance n'est pas :

❌ Un simple agenda
❌ Une application de notes
❌ Un outil de facturation uniquement

Cadance est :

✅ Un assistant métier complet pour chauffeur VTC.

---

# 👥 Segmentation utilisateurs

---

# 1. Chauffeur indépendant

Profil :

- travaille seul
- possède son véhicule
- gère ses propres clients

Besoin :

- organisation
- facturation
- suivi revenus

---

# 2. Chauffeur professionnel premium

Profil :

- beaucoup de courses privées
- clientèle régulière
- besoin d'automatisation

Besoin :

- CRM
- statistiques
- automatisations

---

# 3. Société VTC

Profil :

- plusieurs chauffeurs
- plusieurs véhicules

Besoin :

- gestion équipe
- planning partagé
- contrôle activité

---

# 💳 Modèle d'abonnement

---

# 🆓 Offre Free

Objectif :

Permettre au chauffeur de découvrir l'application.

---

Fonctionnalités :

## Gestion

- 50 courses/mois
- 50 clients
- planning simple

---

## Documents

- Historique limité
- Export simple

---

## Dashboard

- Statistiques basiques

---

Limitation :

- Pas d'automatisation
- Pas d'IA
- Pas de multi véhicule

---

# ⭐ Offre Premium

Cœur du business.

Prix conseillé :

```
9,99€ / mois
```

ou

```
99€/an
```

---

Fonctionnalités :

## Courses

- Courses illimitées
- Historique complet
- Types avancés

---

## Clients

- Clients illimités
- CRM complet
- Notes privées

---

## Planning

- Planning avancé
- Détection conflits

---

## Facturation

- Factures PDF
- Signature numérique
- Envoi automatique

---

## Finance

- Dépenses
- Rentabilité
- Statistiques avancées

---

## Automatisation

- SMS rappel
- Emails
- Notifications intelligentes

---

## IA

Crédits IA inclus.

---

# 🚀 Offre Business

Pour entreprises VTC.

Prix :

```
À partir de 29,99€/mois
```

---

Fonctionnalités :

Tout Premium + :

---

## Gestion équipe

- Ajout chauffeurs
- Rôles utilisateurs
- Permissions

---

## Planning partagé

- Attribution courses
- Disponibilités

---

## Véhicules

- Flotte complète
- Maintenance

---

## Statistiques entreprise

- CA global
- Performance chauffeurs
- Rentabilité flotte

---

# 🏢 Offre Enterprise

Pour grandes structures.

Prix personnalisé.

---

Fonctionnalités :

- API dédiée
- Support prioritaire
- Intégrations personnalisées
- SLA
- Déploiement spécifique

---

# 💵 Revenus supplémentaires

---

# 1. Crédit IA

L'utilisateur peut acheter des crédits supplémentaires.

Exemple :

```
100 analyses IA = 4,99€
```

---

# 2. Services partenaires

Possibilité de partenariat avec :

- assurances VTC
- garages
- stations carburant
- leasing automobile

---

# 3. Marketplace

Future évolution.

Exemples :

- entretien véhicule
- nettoyage voiture
- accessoires chauffeur

---

# 4. Commission paiement

Future possibilité :

Paiement intégré des courses.

Commission :

```
1-2%
```

---

# 📈 Stratégie acquisition

---

# Phase 1

Objectif :

Obtenir les premiers utilisateurs.

Méthodes :

- groupes Facebook VTC
- communautés chauffeurs
- TikTok
- YouTube
- bouche à oreille

---

# Phase 2

Développement communauté.

Créer :

- contenus conseils
- statistiques marché VTC
- guides chauffeur

---

# Phase 3

Partenariats.

Avec :

- écoles VTC
- entreprises transport
- plateformes indépendantes

---

# 🎁 Stratégie lancement

---

# Essai gratuit

Premium offert :

```
30 jours
```

---

Objectif :

Créer une habitude d'utilisation.

---

# Parrainage

Un chauffeur invite un autre chauffeur.

Récompense :

- 1 mois gratuit
- crédits IA

---

# KPI importants

---

## Acquisition

- nouveaux comptes
- coût acquisition client

---

## Activation

Pourcentage utilisateurs ayant créé :

- premier client
- première course
- première facture

---

## Rétention

Nombre utilisateurs actifs :

- quotidien
- hebdomadaire
- mensuel

---

## Revenus

Suivre :

- MRR
- ARR
- Churn
- Conversion Free → Premium

---

# 📊 Objectifs hypothétiques

## 100 utilisateurs Premium

```
100 × 9,99€

≈ 999€/mois
```

---

## 1000 utilisateurs Premium

```
1000 × 9,99€

≈ 9990€/mois
```

---

## 5000 utilisateurs Premium

```
5000 × 9,99€

≈ 49950€/mois
```

---

# 🧠 Philosophie tarifaire

Le prix doit rester inférieur au coût administratif généré par l'absence d'outil.

Un chauffeur gagne du temps et améliore sa rentabilité.

L'application doit être perçue comme :

"Un investissement qui me rapporte plus qu'il ne me coûte."

---

# Conclusion

Cadance doit évoluer d'une application mobile vers un véritable SaaS métier pour les professionnels du transport.

Le modèle abonnement permet une croissance durable tout en restant accessible aux chauffeurs indépendants.
